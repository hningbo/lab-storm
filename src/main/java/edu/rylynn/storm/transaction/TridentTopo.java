...
public class TridentTopo {
    ...
    private static TridentTopology buildTopology() {
        ...
        topology.newStream("kafkaSpout", fixedBatchSpout).
                each(new Fields("order"), new getMerchantPrice(), new Fields("merchantName", "price")).     //这里对订单进行分割，得到[订单名，价格]的格式
    ...
    private static class getTotalPrice implements CombinerAggregator<Float> {
        ...
        @Override
        public Float combine(Float val1, Float val2) {
            float val = val1 + val2;
            return val;
        }
